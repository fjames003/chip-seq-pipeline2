...
if [[ "$#" -lt 7 ]]; then
    echo "USAGE spike_calibrate.sh genome.bed spike_genome.bed scale output(bg|bga|d) genome_chr_lens min_len max_len"
...
output="${name}.${min_len}-${max_len}.${report}"
